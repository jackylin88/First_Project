...
def run():
    ...
    if counter==4:
        print ("Program terminal due to the inactive client")
    else:
        print ("Browser Opening")
        browser = webdriver.Chrome()
        browser.get("http://admin:admin@"+client_IP+"/index.htm#../template/wireless/statistics.shtml")
        z=0
        while z==0:
            counter=0
            for i in range (5):
            ...
            if counter!=5:
                browser.refresh()
                STA_information=browser.find_element_by_xpath('//*[@id="ajax-content"]/div[5]/div/div/div[2]/table/tbody').text
